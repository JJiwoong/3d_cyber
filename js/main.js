...
// 수업
 for(let i=0; i<(frameCount + 500);i++){
    let img=new Image();
    if(i>(frameCount - 1)){
 ...
 gsap.to(imageSeq,{
    ...
    snap:"frame", //"fream은 프레임 단위로 값을 맞추겠다는 의미"
    ease:"none",
    scrollTrigger:{
        trigger:"#page>canvas",
        scrub:"0.15",
...
onUpdate:render//gsap.to가변할때마다 업데이트가 일어남
 })
 ...
 function scaleImage(img,ctx){
    ...
    let ratio=Math.max(hRatio,vRatio);//max(값1,값2) //값1,값2중 큰 값 반대로 min(값1,값2)는 값1,값2중 작은값을 찾는다
